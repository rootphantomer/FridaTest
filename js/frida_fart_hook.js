...
function hookart() {
  ...
  if (module_libext != null) {
    ...
    funcGetCodeItemLength = new NativeFunction(addrGetCodeItemLength, "int", [
      "pointer",
    ]);
    addrBase64_encode = module_libext.findExportByName("Base64_encode");
    funcBase64_encode = new NativeFunction(addrBase64_encode, "pointer", [
      "pointer",
      "int",
      "pointer",
    ]);
  ...
  for (var i = 0; i < symbols.length; i++) {
    ...
    if (
      symbol.name.indexOf("ClassLinker") >= 0 &&
    ...
    Interceptor.attach(addrLoadMethod, {
      ...
      onLeave: function (retval) {
        ...
        if (this.dexfileptr != null) {
          dexfilebegin = Memory.readPointer(
            ptr(this.dexfileptr).add(Process.pointerSize * 1)
          );
          dexfilesize = Memory.readU32(
            ptr(this.dexfileptr).add(Process.pointerSize * 2)
          );
...
function dumpcodeitem(artmethodobj) {
  if (artmethodobj != null) {
    ...
    if (dex_code_item_offset_ != null && dex_code_item_offset_ > 0) {
      var dir = savepath;
      var file_path =
        dir + "/" + dexfilesize + "_" + Process.getCurrentThreadId() + ".bin";
      ...
      if (file_handle && file_handle != null) {
        ...
        if ((codeitemlength != null) & (codeitemlength > 0)) {
          ...
          var base64ptr = funcBase64_encode(
            ptr(codeitemstartaddr),
            codeitemlength,
            ptr(base64lengthptr)
          );
          var b64content = ptr(base64ptr).readCString(
            base64lengthptr.readInt()
          );
  ...
  if (Java.available) {
    Java.perform(function () {
      try {
        ...
        var basedexclassloaderobj = Java.cast(
          classloaderobj,
