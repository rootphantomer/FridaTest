      ...
      Process.enumerateRanges("r--").forEach(function (range) {
        try {
          Memory.scanSync(
            ...
            if (
              range.file &&
              range.file.path && // range.file.path.startsWith("/data/app/") ||
            ...
            if (verify(match.address, range, false)) {
              var dex_size = get_dex_real_size(
                match.address,
                range.base,
          ...
          if (enable_deep_search) {
            Memory.scanSync(range.base, range.size, "70 00 00 00").forEach(
              function (match) {
                ...
                if (
                  dex_base.readCString(4) != "dex\n" &&
                  verify(dex_base, range, true)
                ) {
                  var real_dex_size = get_dex_real_size(
                    dex_base,
                    range.base,
                    range.base.add(range.size)
                  );
